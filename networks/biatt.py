...
class BiAttn(nn.Module):
    ...
    def similarity(self, s1, l1, s2, l2):
        ...
        if self.method == "original":
            ...
            packed_s1_s2 = torch.cat(
                [repeat_s1, repeat_s2, repeat_s1 * repeat_s2], dim=3)  # [B, T1, T2, D*3]
            # s is the similarity matrix from biDAF paper. [B, T1, T2]
    ...
    def get_h_tile(cls, s, s1):
        ...
        # print(b_weight.shape, s1.shape)
        h_tile = torch.bmm(b_weight, s1).repeat(
            1, t1, 1)  # repeat to match s1 # [B, t1, D]
        return h_tile
