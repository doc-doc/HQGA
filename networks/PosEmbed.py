...
def build_graph(bbox, spatial, label_num=11):
    ...
    # [num_boxes, 1]
    image_w, image_h = spatial[0], spatial[1]
...
def build_spatial_graph(bboxes, spatial):

    nclips, nframes_pclip, nbbox_pframe, bbox_dim = bboxes.shape
    total_fnum = nclips * nframes_pclip
    bboxes = bboxes.reshape([total_fnum, nbbox_pframe, bbox_dim])
    adjs = []
    for fid, fbbox in enumerate(bboxes):
        cur_graph = build_graph(fbbox, spatial)
        cur_graph = torch.from_numpy(cur_graph).unsqueeze(0)
        bd_cur_graph = torch_broadcast_adj_matrix(cur_graph)
        # print(bd_cur_graph)
        adjs.append(cur_graph)
        # break

    return torch.from_numpy(np.asarray(adjs))
